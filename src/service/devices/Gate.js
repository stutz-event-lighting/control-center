...
var Gate = module.exports = function Gate(button,io_closed,io_open,relay_open,relay_close,mainlight,outdoorlight){
    Device.call(this);
    this.direction = "up";
    this.button = button;
    ...
    this.io_closed.on("change",function(){
        if(!this.io_closed.value){
            ...
            if(mainlight.state.status == "off") mainlight.turnHalfOn(function(){});
            if(checkDark() && !outdoorlight.state.on) outdoorlight.turnOn(function(){});
        }
    }.bind(this))
}
